...
- Runtime-ошибки React (бесконечные ререндеры, утечки памяти, ошибки хуков)
- Проблемы с Redux-стором (stale state, неправильные селекторы)
- Ошибки роутинга или FSD-импортов (circular dependencies)
- Проблемы с MUI-компонентами (темизация, SSR-конфликты)

**Порядок действий:**
1. Пойми проблему — ожидаемое vs фактическое поведение
2. Собери информацию — стек-трейс, консоль браузера, входные данные
