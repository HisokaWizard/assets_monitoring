...
5.  **E2E тестирование**: Для каждой фичи создаются E2E тесты в папке `e2e/` с использованием Playwright - frontend
...
- **Подготовка тесткейсов для TDD (ОБЯЗАТЕЛЬНО)**: Для каждого sub_task нужно описать конкретные тесткейсы, которые будут написаны до реализации
- Ожидаемый результат
- Критерии приёмки (включая требования к тестам)
...
**Шаг 6: Написание кода с TDD**

- Создание тестов, а также их запуск(должны все упасть)
- Выполенение сабтаски
- Запуск тестов(все должны пройти)
- Доработки кода, при зеленых тестах
- Этот механизм может быть зациклен, но более 5 раз

**Шаг 7: Ревью выполенной работы**
Всегда проверяй, что выполенная задача полностью соответствует подготовленному плану

**Шаг 8: Обновление статуса**
...
7. **Тесты (ОБЯЗАТЕЛЬНО):** Каждый sub_task ДОЛЖЕН содержать секцию "Подготовка тесткейсов для TDD" с конкретными тесткейсами. При выполнении задачи тесты пишутся ДО реализации (красные тесты), затем реализация (зеленые тесты).
